...
> = (set, get) => ({
  ...
  //       const newVariant = product?.variants.find((v) => v.id === newColorId);

  //       if (!newVariant) {
  //         console.error(
  //           `Color variant ${newColorId} not found for product ${cartItem.id}`
  //         );
  //         return;
  //       }
      ...
      state.cartItems = state.cartItems
        .map((cartItem) => {
          ...
          if (!isValidColor || !isValidSize) {
            ...
            const newSelectedColor = firstVariant?.id;
...
//       const selectedVariant = product.variants.find(
//         (v) =>
//           v.color.toLowerCase() === product.selectedColor?.toLocaleLowerCase()
//       );
//       if (!selectedVariant) {
//         console.log("No variant selected or variant not found");
//         return;
//       }
...
//         !selectedVariant.sizes.includes(product.selectedSize)
//       ) {
//         console.log("Selected size not available in this variant");
//         return;
//       }
...
//         id: product._id,
//         colors: product.variants?.map((variant: any) => variant.color) || [],
//         selectedColor: product.variants?.[0]?._id || undefined,
//         mainImage: product.images?.[0]?.url || product.mainImage || "",
...
//   syncBookmarksWithProducts: () => void;
//   getProductById: (productId: string) => ProductData | undefined;
//   getVariantByColorId: (product: ProductData, colorId: string) => ProductData["variants"][0] | undefined;
//   getAvailableColorsForProduct: (productId: string) => { id: string; color: string; colorHex?: string }[];
//   getAvailableSizesForColor: (productId: string, colorId: string) => string[];
// };
...
//     const variant = product.variants.find((v) => v.color.toLowerCase() === colorId);
//     return variant ? variant.sizes : [];
//   },
...
//         // SCENARIO 1: Item exists in cart - update cart item + product
//         const product = state.products.find((p) => p.id === cartItem.id);
//         const newVariant = product?.variants.find((v) => v.color.toLowerCase() === newColor);

//         if (!newVariant) {
//           console.log(`Color variant ${newColor} not found for product ${cartItem.id}`);
//           return;
//         }
...
//           // Check if current size is available in new color variant
//           if (cartItem.selectedSize && !newVariant.sizes.includes(cartItem.selectedSize)) {
//             // Reset to first available size if current size not available
//             cartItem.selectedSize = newVariant.sizes[0] || undefined;
//             cartItem.cartItemId = generateCartItemId(
//               cartItem.id,
...
//           // Update size if needed
//           if (product.selectedSize && !newVariant.sizes.includes(product.selectedSize)) {
//             product.selectedSize = newVariant.sizes[0] || undefined;
//           }
//         }
//       } else {
...
//         // SCENARIO 1: Item exists in cart - update cart item + product
//         const product = state.products.find((p) => p.id === cartItem.id);
//         const currentVariant = product?.variants.find((v) => v.color.toLowerCase() === cartItem.selectedColor);

//         if (!currentVariant || !currentVariant.sizes.includes(newSize)) {
//           console.log(`Size ${newSize} not available for current color variant`);
//           return;
//         }
...
//         // SCENARIO 2: Update product (before adding to cart)
//         const product = state.products.find((p) => p.id === identifier);
//         const currentVariant = product?.variants.find((v) => v.color.toLowerCase() === product.selectedColor);

//         if (product && currentVariant && currentVariant.sizes.includes(newSize)) {
//           product.selectedSize = newSize;
//         }
//       }
//     }),
...
//           // Find the variant for the selected color
//           const selectedVariant = product.variants.find(
//             (v) => v.color.toLowerCase() === cartItem.selectedColor
//           );
...
//           const isValidSize =
//             !cartItem.selectedSize ||
//             (selectedVariant && selectedVariant.sizes.includes(cartItem.selectedSize));
...
//             return {
//               ...product, // Fresh product data
//               cartItemId: generateCartItemId(product.id, newSelectedColor, newSelectedSize),
//               quantity: Math.min(cartItem.quantity, product.stock),
//               selectedColor: newSelectedColor,
//               selectedSize: newSelectedSize,
//             };
...
//           // Find the variant for the selected color
//           const selectedVariant = product.variants.find(
//             (v) => v.color === bookmark.selectedColor
//           );
...
//           const isValidSize =
//             !bookmark.selectedSize ||
//             (selectedVariant && selectedVariant.sizes.includes(bookmark.selectedSize));
...
//           const updatedBookmark: BookmarkType = {
//             ...product, // Fresh product data
//             bookmarkId: generateBookmarkId(product.id, newSelectedColor, newSelectedSize),
//             bookmarkCreatedAt: bookmark.bookmarkCreatedAt,
//             selectedColor: newSelectedColor,
//             selectedSize: newSelectedSize,
//             // Update sizes array to match the selected color
//             sizes: (selectedVariant || firstVariant)?.sizes || [],
//           };
...
//         ...product,
//         id: product._id,
//         selectedColor: product?.variants?.[0]?.id || undefined,
//         selectedSize: product?.variants?.[0]?.sizes?.[0] || undefined,
//         mainImage: product.images?.[0]?.url || product.mainImage || "",
//         sizes: product.sizes || [],
//       }));
