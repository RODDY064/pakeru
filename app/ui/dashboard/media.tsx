  ...
  ({
    ...
    const addColor = useCallback(() => {
      ...
      if (
        variants.some(
          (variant) =>
            variant.name.toLowerCase() === newColor.name.toLowerCase()
        )
      ) {
    ...
    const handleNewColorNameChange = useCallback(
      (e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                <div className="text-center cursor-pointer">
                  ...
                  {needsMoreImages && (
                    <p className="text-sm font-avenir text-orange-600 mt-2">
                      Need {MIN_IMAGES - activeColor.images.length} more
                      image(s) minimum
                    </p>
                  )}
                </div>
